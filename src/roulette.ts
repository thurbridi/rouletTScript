class Game {
    ...
    private processRound(): void {
        this.players.forEach((player) => {
            player.placeBet(Math.floor(37 * Math.random()),
                player.budget - Math.floor(Math.random() * player.budget));
        });
...
class Player {
    ...
    public placeBet(num: number, value: number): void {
        this.currentBet = num;
...
const game = new Game(new FrenchRoulette(), 1000,
